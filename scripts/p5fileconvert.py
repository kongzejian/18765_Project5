...
def read_stil(infilename):
    ...
    for line in infile:  # reads file line by line
        ...
        # finds input vectors
        inputvec = re.search(r"_pi_ =", line)
        if inputvec:
            vecs = []
        ...
        outputvec = re.search(r"_po_ =", line)
        if outputvec:
            vecs = []
...
def read_ascii(infname):
    ...
    # clk_input  = input("Name of clock signal(s) separated by commas, put NONE if circuit is combinational: ")
    # clk_input = clk_input.replace(" ", "")
    # cknames = clk_input.split(",")
    # if cknames[0] == "NONE":
    #     cknames =[]
    ...
    for line in infile:
        if force_flag == 1:
            if ";" in line:
                myVector = myVector + line.strip()[:-4]
                force_flag = 0
                invecs.append(myVector)
            else:
                myVector = myVector + line.strip()[:-1]

        elif measure_flag == 1:
            if ";" in line:
                myResponse = myResponse + line.strip()[:-4]
                measure_flag = 0
                outvecs.append(myResponse)
            else:
                myResponse = myResponse + line.strip()[:-1]
        ...
        elif "declare output bus" in line:
            pos_eq = line.index("=") + 1
            line = line[pos_eq:]
            if "/" in line:
                line = line.replace("/", "")
        ...
        # test vector
        elif "force" in line and "PI" in line:
            myVector = line.split()[2][1:-1]
            if ";" in line:
                invecs.append(myVector)
            else:
                force_flag = 1
        ...
        # response
        elif "measure" in line and "PO" in line:
            myResponse = line.split()[2][1:-1]
            if ";" in line:
                outvecs.append(myResponse)
            else:
                measure_flag = 1
    ...
    print(f"inputs: {innames}")
    print(f"outputs: {outnames}")
    print(f"number of inputs: {len(innames)}")
    print(f"number of outputs: {len(outnames)}")
    print(f"input vectors: {len(invecs[0])}")
    print(f"output vectors: {len(outvecs[0])}")
...
def read_etr(infname):
    ...
    for line in infile:  # reads file line by line
        ...
        if vector:
            vecs = vector.group(1).split()  # separates by grouping
            tempin = []
            tempout = []
...
def write_tbdpatt(
    cknames, innames, indict, intype, outnames, outdict, outfname, infname
):
    """Writes TBDpatt vector file"""
    ...
    for ctb in range(len(outnames)):
        if ctb == 0:
            ...
            outf.write(
                f'#      PO {ctb+1} = "Pin.f.l.MODULE.nl.{outnames[ctb]}")   #  index = {ctc}\n'
            )
        else:
            outf.write(
                f'#      PO {ctb+1} = "Pin.f.l.MODULE.nl.{outnames[ctb]}",   #  index = {ctc}\n'
    ...
    outf.write(
        " [ Test_Section 1.1 (tester_termination = none,  "
        + "test_section_type = logic, test_type = static, simulated);\n"
    )
    outf.write("   [ Keyed_Data;\n")
    outf.write(
        '    "SimOptions" = "simulation=hsscan 01hot=no choppers=safe "'
    ...
    outf.write("      [ Keyed_Data;\n")
    outf.write(
        '       "AnalysisSimOptions" = "useppis=no simwaitosc=no '
    ...
    for tsct in range(bool(cknames), len(indict[indict.keys()[0]])):
        outf.write(
            "    [ Test_Sequence 1.1.1.2.%d (); # Test %d\n" % (tsct + 1, tsct + 1)
        )
        ...
        if pulseflag:
            outf.write(
                "     [ Pattern 1.1.1.2.%d.3 (pattern_type = static);\n" % (tsct + 1)
            )
        ...
        # Stim_PI event
        outf.write(
            "     [ Pattern 1.1.1.2.%d.1 (pattern_type = static);\n" % (tsct + 1)
        )
        ...
        # Measure_PO event
        outf.write(
            "     [ Pattern 1.1.1.2.%d.2 (pattern_type = static);\n" % (tsct + 1)
        )
...
def write_stilvec(
    cknames, innames, invecs, indict, intype, outnames, outvecs, outfname, infname
):
    ...
    outf.write(f'  Include "{outfname}.signals.stil";\n\n')
    ...
    for outp in outnames:  # output signals
        outf.write(
            "        "
            " + outp + "
            ' { LHTX { "0ns" X/X/X/X;' + ' "72.000000ns" L/H/T/X; } }\n'
        )
    outf.write("  } }\n}\n\n")
...
def write_stilsig(cknames, innames, invecs, intype, outnames, outfname, infname):
    ...
    outf.write(f"    \"ALLPIs\" = \"{'+'.join(cknames + innames)}\";\n")
    outf.write(f"    \"ALLPOs\" = \"{'+'.join(outnames)}\";\n  }}\n\n")
...
def write_vlogtb(
    cknames, innames, invecs, indict, intype, outnames, modname, outfname, infname
):
    ...
    outf.write(f"// {outfname}\n")
    outf.write(f"// Input file: {infname}\n")
    outf.write("// Converted from " + intype + " to Verilog TestBench\n//\n")
    outf.write(f"// Module {modname}\n")
    ...
    outf.write("\treg " + ", ".join(cknames + innames) + ";\t//input signals\n\n")
    ...
    for i in range(len(invecs)):
        for cksig in cknames:
            if indict[cksig][i] == "P":  # Performs clock pulse if defined
                ...
                outf.write("\t\t" + cksig + " = 1'b" + indict[cksig][i] + ";\n")
...
# Writes out VCD file given data from input file
def write_vcd(
    cknames: list[str],
    innames: list[str],
    invecs: list,
    indict: dict[str, list],
    intype: list[str],
    outnames: list[str],
    outdict: dict[str, list],
    modname: str,
    outfname: Path,
    infname: Path,
):
    """Writes Verilog TestBench file"""
    outf = open(outfname, "w")
    outf.write("$version Generated by etr_to_vcd.py $end\n")
    outf.write("$timescale 1ps $end\n")  # timescale

    outf.write("\n")
    # file info
    outf.write("$comment VERILOG TESTBENCH $end\n")
    outf.write(f"$comment {outfname} $end\n")
    outf.write(f"$comment Input file: {infname} $end\n")
    outf.write(f"$comment Converted from {intype} to VCD $end\n")
    outf.write(f"$comment {modname} $end\n")

    # circuit/vectors information
    outf.write(f"$comment {len(invecs)} vectors in file $end\n")
    outf.write(f"$comment {len(cknames)} clock signals present $end\n")
    outf.write(
        f"$comment {len(innames) - len(cknames)} input signals present (not including clocks) $end\n"
    )
    outf.write(f"$comment {len(outnames)} output signals preset $end\n$end\n")

    if cknames:
        outf.write(f"$comment CLOCKS: {', '.join(cknames)} $end\n")
    outf.write(f"$comment INPUTS: {', '.join(innames)} $end\n")
    outf.write(f"$comment OUTPUTS: {', '.join(outnames)} $end\n")

    outf.write("\n\n")

    # define I/O signals
    outf.write("$scope module TOP $end\n")
    outf.write(f"\t$scope module {modname} $end\n")
    for sig in cknames + innames + outnames:
        outf.write(f"\t\t$var wire 1 {sig} {sig} $end\n")
    outf.write("\t$upscope $end\n")
    outf.write("$upscope $end\n")

    outf.write("\n\n")
    outf.write("$enddefinitions $end\n")
    outf.write("$dumpvars\n")
    for sig in cknames + innames + outnames:
        outf.write(f"X {sig}\n")
    outf.write("$end\n\n")
    ...
    for i in range(len(invecs)):
        for cksig in cknames:
            if indict[cksig][i] == "P":  # Performs clock pulse if defined
                outf.write(f"0 {cksig}\n")
            else:
                outf.write(f"{indict[cksig][i]} {cksig}\n")

        # Changes input values
        outf.write(f"\n$comment Test vector {i+1} $end\n")
        for sig in innames:
            outf.write(f"{indict[sig][i]} {sig}\n")
        for sig in outnames:
            outf.write(f"{outdict[sig][i]} {sig}\n")

        outf.write("#5\n")

        for cksig in cknames:
            if indict[cksig][i] == "P":
                outf.write(f"1 {cksig}\n")
...
def write_etr(
    ...
    outf = open(outfname, "w")  # resulting file

    # File info
    outf.write("EASY-TO-READ ** do not remove, file type detection\n")
    outf.write(f"-- {outfname} --\n")
    outf.write(f"Initial input: {infname}\n")
    ...
    outf.write("INPUTS: " + ", ".join(innames) + "\n")
    outf.write("OUTPUTS: " + ", ".join(outnames) + "\n")
    outf.write("Ordering of signals corresponds to order of bits in test " + "vector\n")
    outf.write("\n\n")
...
def write_etrmap(innames, outnames, outfname):
    ...
    outf.write(
        "File used to define scan chains for use with"
    ...
    outf.write(
        "** EACH OF THE ABOVE SIGNALS MUST BE ASSIGNED TO ONE OF THE "
        + "FOLLOWING GROUPS **\n"
    )
    ...
    outf.write(
        '** If "A_in" (PO of comb circuit) is the serial '
        + "input to the scan flip flop and\n"
    )
    outf.write(
        '**    "A_out" (PI of comb circuit) is the serial '
    ...
    outf.write("** Clock signals will contain an initial 0 followed by" + " pulses.\n")
    outf.write("** Control signals can be assigned a cycle using the form:\n")
    outf.write('**\t"CNTLA=1, CNTLB=1110, CNTLC=ABC"\n')
    outf.write(
        "** If value is not assigned for control signal the"
        + ' value "%" will be given\n'
    )
    outf.write("CLOCKS: \n")
    outf.write("CONTROLS: \n\n\n")
    ...
    outf.write(
        "** How many filler vectors should print before stimulating"
        + " PIs or reading POs?\n"
    )
    ...
    outf.write("** Scan in port and scan out port will be created" + " automatically\n")
    ...
    outf.write(
        "** For internal scan FFs give only the serial input "
        + "signal (comb circuit output)\n"
    )
    outf.write(
        '** Reference help document "Project 5 Scripts Help" for '
...
def write_ascii(
    ...
    with open(outfname, "w") as writefd:
        writefd.write("ASCII_PATTERN_FILE_VERSION = 3;\n\nSETUP =\n")
        # define PIs
        write_str = '     declare input bus "PI" ='
        for pi in cknames + innames:
            write_str = write_str + " " / " + pi  + " ","
        write_str = write_str[:-1] + ";\n\n"
        writefd.write(write_str)

        # define POs
        write_str = '     declare output bus "PO" ='
        for po in outnames:
        ...
        for i in range(len(invecs)):
            writefd.write("    pattern = " + str(i) + " clock_sequential ;\n")
            # writefd.write("    force    "PI" "")
            in_str = ""
            for sig in cknames + innames:
                if sig in cknames:
                    in_str = in_str + "0"
                    ...
                    in_str = in_str + indict[sig][i]
            in_str = textwrap.wrap(in_str, 50)
            if len(in_str) == 1:
                writefd.write(f'    force   "PI" "{in_str[0]}" 0;\n')
            elif len(in_str) > 1:
                writefd.write(f'    force   "PI" "{in_str[0]}\\')
            ...
            for j in range(0, len(cknames)):
                writefd.write(f'    pulse "{cknames[j]}" 1;\n')

            output_pat = outvecs[i]
            output_pat = textwrap.wrap(output_pat, 50)

            if len(output_pat) == 1:
                writefd.write(f'    measure "PO" "{output_pat[0]}" 1;\n\n')
            if len(output_pat) > 1:
                writefd.write(f'    measure "PO" "{output_pat[0]}\\')
